...
public class SockServer {
  ...
  public static class Movie {
    ...
    public boolean hasUserRated(String user) {
      return this.ratings.containsKey(user);
    }
      ...
      while (true){
        ...
        while (connected) {
          ...
          if (req.getString("type").equals("echo")) {
            ...
            res = addmany(req);
          } else if (req.getString("type").equals("rating")) {
            res = processRatingRequest(req);
          } else {
            res = wrongType(req);
          }
          writeOut(res);
  ...
  static JSONObject addMovie(JSONObject req) {
    JSONObject res = new JSONObject();
    String movieTitle = req.getString("movie");
    int rating = req.getInt("rating");
    String username = req.getString("username");

    if (movies.containsKey(movieTitle)) {
      res.put("ok", false);
      res.put("message", "Movie has already been added.");
      return res;
    }

    Movie newMovie = new Movie(movieTitle);
    newMovie.addRating(username, rating);
    movies.put(movieTitle, newMovie);

    res.put("ok", true);
    return res;
  }

  static JSONObject viewRating(JSONObject req) {
    JSONObject res = new JSONObject();
    String movieTitle = req.optString("movie", "");
    ...
    if (movieTitle.isBlank()) {
      ...
      for (Movie movie : movies.values()) {
        ...
        movieJson.put("rating", movie.getAvgRating());
        moviesArray.put(movieJson);
      }
      res.put("movies", moviesArray);
      res.put("ok", true);
    } else {
      if (!movies.containsKey(movieTitle)) {
        res.put("ok", false);
        res.put("message", "Movie has not yet been added.");
        return res;
      }

      Movie movie = movies.get(movieTitle);
      res.put("ok", true);
      res.put("movie", movie.getTitle());
      res.put("rating", movie.getAvgRating());
    }
    return res;
  }

  static JSONObject addRating(JSONObject req) {
    JSONObject res = new JSONObject();
    String movieTitle = req.getString("movie");
    String username = req.getString("username");
    int rating = req.getInt("rating");
    ...
    Movie movie = movies.get(movieTitle);
    if (movie.hasUserRated(username)) {
      res.put("ok", false);
      res.put("message", "You already provided a rating for this movie.");
      return res;
    }
  ...
  static JSONObject processRatingRequest(JSONObject req) {
    ...
    if (task.equals("add")) {
      ...
      res = viewRating(req);
    } else if (task.equals("rate")) {
      res = addRating(req);
    } else {
      res.put("ok", false);
      res.put("message", "Task " + task + " is not supported.");
    }
    return res;
  }

  static JSONObject processMoviesRequest(JSONObject req) {
    JSONObject res = new JSONObject();
    JSONArray moviesArray = new JSONArray();
    for (Movie movie : movies.values()) {
      JSONObject movieJson = new JSONObject();
  ...
  static void processConnectedClients(JSONObject req) {
    JSONObject res;
  ...
  static JSONObject addmany(JSONObject req){
    ...
    if (array.length() ==1) {
      res.put("ok", false);
      res.put("message", "Only one value given");
      return res;
    }
